...
{
	float beforeDepth = texelFetch(depthTex, ivec2(gl_FragCoord.xy) - dir, 0).x;
	float afterDepth = texelFetch(depthTex, ivec2(gl_FragCoord.xy) + dir, 0).x;
...
{
	vec2 zoverxyBefore = vec2(depthDerivate(ivec2(gl_FragCoord.xy) - dir, ivec2(1, 0)), depthDerivate(ivec2(gl_FragCoord.xy) - dir, ivec2(0, 1)));
	vec2 zoverxyAfter = vec2(depthDerivate(ivec2(gl_FragCoord.xy) + dir, ivec2(1, 0)), depthDerivate(ivec2(gl_FragCoord.xy) + dir, ivec2(0, 1)));
...
void main()
{
	...
	vec2 f = vec2(tan(3.1415 / 2) / v.x, tan(3.1415 / 2) / v.y);
	...
	vec3 Nxy = texture2D(normalTex, uv).xyz;
